...
export default async function PopulationTable({ optimizationResults }: PopulationTableProps) {
  ...
  const prefecturePopulations = data.reduce((acc, row) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.info}>
        <p>データ件数: {data.length}件 | グループ数: {optimizationResults.bestPartition.length}つ</p>
        <p>友好度合計スコア: <strong>{optimizationResults.totalScore.toFixed(1)}</strong></p>
      </div>
          ...
          <thead>
            <tr>
              ...
              {prefectures.map(prefecture => (
                <th key={prefecture} className={`${styles.tableHeader} ${styles[getGroupClass(prefecture)]}`}>
          ...
          <tbody>
            {years.map(year => (
              <tr key={year} className={styles.tableRow}>
                ...
                {prefectures.map(prefecture => (
                  <td key={`${year}-${prefecture}`} className={`${styles.tableCell} ${styles[getGroupClass(prefecture)]}`}>
