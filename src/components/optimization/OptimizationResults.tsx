...
import { OPTIMIZATION_RESULTS_TITLE, TOTAL_GROUPS_LABEL, TOTAL_SCORE_LABEL } from "./constants";
...
export default function OptimizationResults({ initialResults }: OptimizationResultsProps) {
  ...
  return (
    <div className={styles.optimizationResults}>
      <h2>{OPTIMIZATION_RESULTS_TITLE}</h2>

      {/* Display optimization stats */}
      <div className={styles.stats}>
        <p>
          <strong>{TOTAL_GROUPS_LABEL}</strong> {bestPartition.length}
        </p>
        <p>
          <strong>{TOTAL_SCORE_LABEL}</strong> {totalScore}
        </p>
      </div>
